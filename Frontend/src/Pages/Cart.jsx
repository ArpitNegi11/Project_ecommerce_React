...
const Cart = () => {
  ...
  const fetchCart = () => {
    axios.get('https://project-ecommerce-react-backend.onrender.com/cart')
      .then((response) => {
  ...
  const reduceQuantity = (productId) => {
    axios.delete(`https://project-ecommerce-react-backend.onrender.com/cart/reduce/${productId}`)
      .then(() => fetchCart())
      .catch((err) => console.error('Error reducing quantity:', err));
  };

  const deleteProduct = (productId) => {
    axios.delete(`https://project-ecommerce-react-backend.onrender.com/cart/delete/${productId}`)
      .then(() => fetchCart())
  ...
  return (
    <div>
      ...
      {cart.length === 0 ? (
        <div className="no"><p>Empty cart</p></div>
      ) : (
